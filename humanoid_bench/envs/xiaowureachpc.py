...
class XiaoWuReach:
    ...
    def get_reward(self):
        ...
        POSITION_THRESHOLD_SMALL = 0.1
        POSITION_THRESHOLD_MEDIUM = 0.2
        POSITION_THRESHOLD_LARGE = 0.3

        VELOCITY_THRESHOLD_SMALL = 0.2
        VELOCITY_THRESHOLD_MEDIUM = 0.3
        VELOCITY_THRESHOLD_LARGE = 0.4
        ...
        def calculate_part_reward(pos_err, vel, part_name):
            ...
            # --- 速度阶段奖励 ---
            if vel <= VELOCITY_THRESHOLD_SMALL:  
                vel_ratio = 1 - vel / VELOCITY_THRESHOLD_SMALL
                precision_vel_bonus = 2 + 2 * vel_ratio                  
            elif vel <= VELOCITY_THRESHOLD_MEDIUM:  
                vel_ratio = 1 - (vel - VELOCITY_THRESHOLD_SMALL) / (VELOCITY_THRESHOLD_MEDIUM - VELOCITY_THRESHOLD_SMALL)
                precision_vel_bonus = 1 + 1 * vel_ratio                  
            elif vel <= VELOCITY_THRESHOLD_LARGE: 
                vel_ratio = 1 - (vel - VELOCITY_THRESHOLD_MEDIUM) / (VELOCITY_THRESHOLD_LARGE - VELOCITY_THRESHOLD_MEDIUM)
                precision_vel_bonus = 0 + 1 * vel_ratio  
            else:  
                precision_vel_bonus = -10 * vel

            precision_bonus = precision_pos_bonus + precision_vel_bonus

            return {
                f"{part_name}_total": pos_bonus + vel_bonus + precision_bonus, 
                f"{part_name}_reached3": pos_err <= 0.05,
                f"{part_name}_reached5": pos_err <= 0.07
            }
        ...
        # 判断是否成功
        self._left_reached3 = elbow_l_reward["elbow_l_reached3"]
        self._right_reached3 = elbow_r_reward["elbow_r_reached3"]
        ...
        left_success_bonus = 5 if self._left_reached5 else 0.0
        right_success_bonus = 5 if self._right_reached5 else 0.0
        success_bonus = 15 if self._is_success else 0.0
    ...
    def reset_model(self):
        ...
        self._locked_left_qpos = None  
        self._locked_right_qpos = None

        self._prev_actuator_force = None
        self._prev_action = None
        self._is_success = False

        self._left_reached3 = False
        self._right_reached3 = False
    ...
    def step(self, action):
        ...
        if self._current_step <= self._initial_zero_steps:
            ...
            # 左臂锁定逻辑
            if self._left_reached3 and self._locked_left_qpos is None:
                self._locked_left_qpos = current_qpos[self._left_arm_indices].copy()
                self._left_permanently_locked = True
            
            if self._left_permanently_locked:
                # 锁定后：禁用平滑
                unnormalized_action[self._left_arm_indices] = self._locked_left_qpos
            else:
                # 未锁定：使用平滑
                target_left = unnormalized_action[self._left_arm_indices]
                current_left = current_qpos[self._left_arm_indices]
                unnormalized_action[self._left_arm_indices] = ((1 - smoothing_factor) * current_left + smoothing_factor * target_left)

            # 右臂锁定逻辑
            if self._right_reached3 and self._locked_right_qpos is None:
                self._locked_right_qpos = current_qpos[self._right_arm_indices].copy()
                self._right_permanently_locked = True

            if self._right_permanently_locked:
                # 锁定后：禁用平滑
                unnormalized_action[self._right_arm_indices] = self._locked_right_qpos
            else:
                # 未锁定：使用平滑过渡
                target_right = unnormalized_action[self._right_arm_indices]
                current_right = current_qpos[self._right_arm_indices]
                unnormalized_action[self._right_arm_indices] = ((1 - smoothing_factor) * current_right + smoothing_factor * target_right)
