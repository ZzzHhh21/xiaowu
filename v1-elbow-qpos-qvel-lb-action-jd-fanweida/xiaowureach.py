...
class XiaoWuReach:
    ...
    def get_reward(self):
        ...
        VELOCITY_THRESHOLD_MEDIUM = 0.4
        VELOCITY_THRESHOLD_LARGE = 0.6
        ...
        def calculate_part_reward(pos_err, vel, part_name):
            ...
            return {
                ...
                f"{part_name}_reached": pos_err <= 0.05
            }
        ...
        # 判断是否成功
        self._left_reached = elbow_l_reward["elbow_l_reached"]
        self._right_reached = elbow_r_reward["elbow_r_reached"]
        self._is_success = self._left_reached and self._right_reached

        left_success_bonus = 5 if self._left_reached else 0.0
        right_success_bonus = 5 if self._right_reached else 0.0
    ...
    def reset_model(self):
        ...
        self._left_reached = False
        self._right_reached = False
    ...
    def step(self, action):
        ...
        if self._current_step <= self._initial_zero_steps:
            ...
            # 左臂锁定逻辑
            if self._left_reached and self._locked_left_qpos is None:
                self._locked_left_qpos = current_qpos[self._left_arm_indices].copy()

            target_left = (self._locked_left_qpos if self._left_reached else unnormalized_action[self._left_arm_indices])
            ...
            # 右臂渐进控制（逻辑相同）
            if self._right_reached and self._locked_right_qpos is None:
                self._locked_right_qpos = current_qpos[self._right_arm_indices].copy()

            target_right = (self._locked_right_qpos if self._right_reached else unnormalized_action[self._right_arm_indices])
