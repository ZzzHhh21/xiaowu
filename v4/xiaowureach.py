...
class XiaoWuReach:
    ...
    def get_reward(self):
        ...
        wrist_l_vel = np.linalg.norm(self.robot.wrist_left_lin_vel())
        wrist_r_vel = np.linalg.norm(self.robot.wrist_right_lin_vel())
        ...
        if elbow_right_height >= head_height:
            height_right_bonus = -5 * np.linalg.norm(elbow_right_height - head_height)
        else:
            height_right_bonus = 0
        
        if elbow_left_height >= head_height:
            height_left_bonus = -5 * np.linalg.norm(elbow_left_height - head_height)
        else:
            height_left_bonus = 0
        ...
        # 分别计算肘部奖励
        def calculate_part_reward(pos_err, vel_elbow, vel_wrist, part_name):
            pos_bonus = -10 * pos_err
            vel_bonus = -1 * vel_wrist

            return {
                f"{part_name}_total": pos_bonus + vel_bonus, 
                f"{part_name}_reached1": pos_err <= 0.05,
                f"{part_name}_reached3": vel_wrist <= 0.05,
                f"{part_name}_reached5": pos_err <= 0.05 and vel_wrist <= 0.05
            }
            
            
        # 计算各部位奖励
        elbow_l_reward = calculate_part_reward(elbow_l_pos_err, elbow_l_vel, wrist_l_vel, "elbow_l")
        elbow_r_reward = calculate_part_reward(elbow_r_pos_err, elbow_r_vel, wrist_r_vel, "elbow_r")

        self._left_reached1 = elbow_l_reward["elbow_l_reached1"]
        self._right_reached1 = elbow_r_reward["elbow_r_reached1"]
        ...
        if self._left_reached1:
            left_success_bonus = 1.5
            if self._left_reached3:
                left_success_bonus = 3
        else:
            left_success_bonus = 0
        
        if self._right_reached1:
            right_success_bonus = 1.5
            if self._right_reached3:
                right_success_bonus = 3
        else:
            right_success_bonus = 0

        
        success_bonus = 8 if self._is_success else 0.0
